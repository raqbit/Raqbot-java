    ...
    private static void sendLines(MessageEvent event, long currentTimestamp, int multiplier, int timeSpan) throws IOException {
        ...
        for (String line : msglist) {
            ...
            } else
            {
                ...
                event.getUser().send().message(Colors.RED + Colors.BOLD + "[MESSAGE BROKEN]" + Colors.NORMAL + " " + getMessage(line) + " " + Colors.RED + Colors.BOLD + "[PLEASE REPORT THIS INCIDENT TO RAQBIT.]");
                found = true;
                sentSpacer = true;
            }
        }
